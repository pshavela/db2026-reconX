...
## Follow-up: wired the fetch in Trades.jsx
- `apiService.js`: implemented `listTrades(params)` as `request('GET', '/v1/trades' + params)`, using the `request()`/`authHeaders()` helper from TICKET-ADV112
- `Trades.jsx`: added the `useEffect([page, debounced])` that builds a query string (`page`, `size=20`, and `status` only when the search box isn't empty), calls `api.listTrades(...)`, stores the response in `data`, and falls back to an empty page on any error — plus rendering the missing `<DataTable.Body rows={data.items} render={...} />` so fetched rows actually show up
- Verified against the **real backend** (not a mock), running under the `dev` profile (H2, no Docker needed):
  - `GET /api/v1/trades?page=0&size=20` returns 200 with the expected `PagedResponse` shape; the page renders the header row + "1 / 1" pagination with zero rows (no seed data) — no console errors
  - Created a real trade via `POST /api/v1/trades` (curl), then reloaded the Trades page — hit a **pre-existing backend bug**: `org.hibernate.LazyInitializationException: Could not initialize proxy [Counterparty#1] - no session` on the list endpoint, already documented separately by a colleague in `tracking-progress/day05/TICKET-ADV063.md` (lazy `instrument`/`counterparty` on `Trade` needs `FetchType.EAGER` to be listable). Not something to fix here
  - Confirmed the important part for this ticket: on that 500, the frontend did **not** crash — it degraded to the empty-table state exactly as designed, proving the `.catch()` fallback works against a real failure, not just a simulated one
