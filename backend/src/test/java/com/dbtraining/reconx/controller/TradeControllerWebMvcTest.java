...
/**
 * TICKET-ADV075 — MockMvc slice test: authenticated trade creation returns 201.
 */
// JwtAuthenticationFilter is a @Component (Filter), so @WebMvcTest normally
// pulls it into the slice's filter chain automatically. Excluding it here
// avoids two problems: its real constructor needs JwtTokenProvider (a plain
// @Component not present in this slice), and mocking the filter instead
// would make it a no-op that never calls chain.doFilter(...), silently
// swallowing every request before it reaches the controller.
